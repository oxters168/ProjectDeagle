...
namespace DemoInfo
{
	...
	public class DemoParser : IDisposable
	{
		...
		/// </summary>
		internal DemoInfo.DP.Entity[] Entities = new DemoInfo.DP.Entity[MAX_ENTITIES]; //Max 2048 entities. 
		...
		internal Vector3 bombsiteACenter, bombsiteBCenter;
		...
		private void HandleNewPlayer(Entity playerEntity)
		{
			...
			playerEntity.FindProperty("cslocaldata.m_vecOrigin").VectorRecived += (sender, e) => {
				p.Position = new Vector3(e.Value.x, e.Value.y, p.Position.z); 
				//p.Position.y = e.Value.y;
			};

			playerEntity.FindProperty("cslocaldata.m_vecOrigin[2]").FloatRecived += (sender, e) => {
				p.Position = new Vector3(p.Position.x, p.Position.y, e.Value); 
			};
			...
			playerEntity.FindProperty("localdata.m_vecVelocity[0]").FloatRecived += (sender, e) => p.Velocity = new Vector3(e.Value, p.Velocity.y, p.Velocity.z);
			playerEntity.FindProperty("localdata.m_vecVelocity[1]").FloatRecived += (sender, e) => p.Velocity = new Vector3(p.Velocity.x, e.Value, p.Velocity.z);
			playerEntity.FindProperty("localdata.m_vecVelocity[2]").FloatRecived += (sender, e) => p.Velocity = new Vector3(p.Velocity.x, p.Velocity.y, e.Value);
			...
			if (equipment.Weapon == EquipmentElement.P2000) {
				e.Entity.FindProperty("m_nModelIndex").IntRecived += (sender2, e2) => {
					...
					if(modelprecache[e2.Value].EndsWith("_pist_223.mdl"))
						...
						throw new Exception("Unknown weapon model");
				};
			}
