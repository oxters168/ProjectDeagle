...
namespace DemoInfo.DP.Handler
{
    public static class PacketEntitesHandler
    {
			...
			for (int i = 0; i < packetEntities.UpdatedEntries; i++) {
				...
				if (!reader.ReadBit()) {
					...
					if (reader.ReadBit()) {
						...
						// preserve / update
						DemoInfo.DP.Entity e = parser.Entities[currentEntity];
						e.ApplyUpdate(reader);
					}
				} else {
					// leave / destroy
		...
		private class PropertyCollector : IDisposable
		{
			...
			private void HandleVectorRecived (object sender, PropertyUpdateEventArgs<Vector3> e) { Capture.Add(e.Record()); }
